...
public class Settings : ISettings
{
    ...
    private static readonly ImmutableList<string> USER_SUFFIX_PATHS = new List<string>
    {
        ...
        // Default for macOS
        "Library/Application Support/Steam/steamapps/common/Hollow Knight Silksong/silksong.app"
    }
    .ToImmutableList();
    // @formatter:on
    ...
    private static bool TryDetectSteamRegistry([MaybeNullWhen(false)] out ValidPath path)
    {
        ...
        path = library_paths.Select(library_path => Path.Combine(library_path, "steamapps", "common", "Hollow Knight Silksong"))
                            .Select(PathUtil.ValidateWithSuffix)
                            .OfType<ValidPath>()
                            .FirstOrDefault();
    ...
    private void DetectLinuxGamePlatform()
    {
        ...
        try
        {
            // Managed -> Hollow Knight Silksong_Data -> Hollow Knight Silksong
            string @base = Path.GetFullPath(Path.Combine(ManagedFolder, "..", ".."));

            Platform = File.Exists(Path.Combine(@base, "Hollow Knight Silksong.exe"))
                // We're on proton.
